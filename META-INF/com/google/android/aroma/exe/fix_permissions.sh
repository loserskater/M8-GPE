...
fp_package()
{
   ...
   myUID=$( $ECHO $pkgline | $SED 's%.*serId="\(.*\)".*%\1%' |  $CUT -d '"' -f1 )
   GID=$myUID
   ...
   #debug
   if $TEST $DEBUG -eq 1; then
      fp_print "CODEPATH: $CODEPATH APPDIR: $APPDIR APK:$APK UID/GID:$myUID:$GID"
   fi
   ...
   if $TEST -d "/data/data/$PACKAGE"; then
      ...
      $FIND /data/data/$PACKAGE -type d -exec $LS -ldn {} \; | while read dataline; do
         ...
         if $TEST "$FOURDIR" = ""; then
            #package directory, perms:755 owner:$myUID:$GID
            ...
            #shared_prefs directories, perms:771 owner:$myUID:$GID
            #shared_prefs files, perms:660 owner:$myUID:$GID
            REVPERM=660
            REVPSTR="rw-rw----"
            fp_chmod $OLD_PER "rwxrwx--x" 771 "$FILEDIR"
            elif $TEST "$FOURDIR" = "databases"; then
            #databases directories, perms:771 owner:$myUID:$GID
            #databases files, perms:660 owner:$myUID:$GID
            REVPERM=660
            REVPSTR="rw-rw----"
            fp_chmod $OLD_PER "rwxrwx--x" 771 "$FILEDIR"
            elif $TEST "$FOURDIR" = "cache"; then
            #cache directories, perms:771 owner:$myUID:$GID
            #cache files, perms:600 owner:$myUID:GID
            REVPERM=600
            REVPSTR="rw-------"
            fp_chmod $OLD_PER "rwxrwx--x" 771 "$FILEDIR"
         else
            #other directories, perms:771 owner:$myUID:$GID
            REVPERM=771
            REVPSTR="rwxrwx--x"
            fp_chmod $OLD_PER "rwxrwx--x" 771 "$FILEDIR"
         fi
         ...
         if $TEST "$ISLIB" = "1"; then
            ...
            fp_chown_uid $OLD_UID $myUID "$FILEDIR"
